...
void *calculateColors(void *threadNumber){
    Vec3 camera_pos;
    ...
    for( int px=*(int *)threadNumber; px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             * Scaled color values are stored in an array
             */
...
main( int argc, char **argv )
{
    ...
    int * threadNumbers = malloc(sizeof(int) * nthreads);
    ...
    {
        threadNumbers[i] = i;
        returnValue = pthread_create(&threads[i], NULL, calculateColors, &threadNumbers[i]);
        if (returnValue)
        {
    ...
    for(int i = 0; i < nthreads; i++) {
        returnValue = pthread_join(threads[i], NULL);
        if (returnValue)
        {
